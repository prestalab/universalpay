{foreach from=$universalpay item=ps}
	<div class="payment_module">
		<a href="{if !$universalpay_onepage}{$link->getModuleLink('universalpay', 'payment', ['id_universalpay_system'=>$ps.id_universalpay_system], true)}{else}#{/if}" title="{$ps.name}" class="universalpay">
			<img src="{$img_ps_dir}pay/{$ps.id_universalpay_system}.jpg" alt="{$ps.name}"/>
			{$ps.description_short}
		</a>
		{if $universalpay_onepage}
		<br/>
		<form action="{$link->getModuleLink('universalpay', 'validation', [], true)}" method="post" class="universalpay_hidden">
		<fieldset>
			{$ps.description}
			<p>
